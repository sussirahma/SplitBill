(() => {
  ...
  function renderStats() {
    ...
    const statMember = document.getElementById('stat-member-count');

    if (statExpense) statExpense.textContent = formatRupiah(totalExpense);
    if (statMember) statMember.textContent = `${state.members.length} Orang`;
  }
  ...
  function updateExpenseCalculations() {
    ...
    if (totalPaidDisplay) totalPaidDisplay.textContent = `Total: ${formatRupiah(totalPaid)}`;
    ...
    if (splitCountText) splitCountText.textContent = `${splitCount} orang terpilih`;
  ...
  function renderHistory() {
    ...
    if (badge) badge.textContent = `${state.transactions.length} Transaksi`;
  ...
  function handleAddMember(e) {
    ...
    if (!name) {
      if (errorText) errorText.textContent = 'Nama anggota tidak boleh kosong!';
      if (errorMsg) {
    ...
    if (isDuplicate) {
      if (errorText) errorText.textContent = `Anggota bernama "${name}" sudah terdaftar!`;
      if (errorMsg) {
        errorMsg.classList.remove('hidden');
        errorMsg.classList.add('flex');
      }
  ...
  function handleSaveExpense(e) {
    ...
    const description = descInput ? (descInput.value.trim() || 'Pengeluaran Bersama') : 'Pengeluaran Bersama';

    if (state.members.length < 2) {
      if (errorText) errorText.textContent = 'Minimal 2 anggota dibutuhkan untuk mencatat pengeluaran.';
      if (errorMsg) {
        errorMsg.classList.remove('hidden');
        errorMsg.classList.add('flex');
      }
    ...
    if (totalPaid <= 0) {
      if (errorText) errorText.textContent = 'Harap masukkan nominal yang dibayar oleh minimal 1 anggota.';
      if (errorMsg) {
        errorMsg.classList.remove('hidden');
        errorMsg.classList.add('flex');
      }
    ...
    if (splitBetween.length === 0) {
      if (errorText) errorText.textContent = 'Pilih minimal 1 orang yang menanggung beban biaya.';
      if (errorMsg) {
        errorMsg.classList.remove('hidden');
        errorMsg.classList.add('flex');
      }
  ...
  function initEventListeners() {
    ...
    if (formAddMember) formAddMember.addEventListener('submit', handleAddMember);
    ...
    const btnSelectAll = document.getElementById('btn-select-all-split');
    if (btnSelectAll) {
    ...
    const btnUnselectAll = document.getElementById('btn-unselect-all-split');
    if (btnUnselectAll) {
    ...
    const btnResetExpense = document.getElementById('btn-reset-expense-form');
    if (btnResetExpense) {
    ...
    if (btnCopyWa) btnCopyWa.addEventListener('click', copyWhatsAppSummary);
    ...
    if (btnCancelClearHistory && modalClearHistory) {
      btnCancelClearHistory.addEventListener('click', () => {
        modalClearHistory.classList.add('hidden');
      });
    }
    if (btnConfirmClearHistory && modalClearHistory) {
      btnConfirmClearHistory.addEventListener('click', () => {
        modalClearHistory.classList.add('hidden');
        clearAllHistory();
      });
    }
